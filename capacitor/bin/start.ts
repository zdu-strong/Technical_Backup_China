...
async function startReact(avaliablePort: number, ReactServerAddress: string) {
  ...
  const childProcessOfWaitOn = execa.command(`wait-on ${ReactServerAddress}`);
...
async function getDeviceList(isRunAndroid: boolean) {
  ...
  if (linq.from(deviceList).where(s => s.includes("Pixel_6_API_")).count() === 1) {
    return linq.from(deviceList).where(s => s.includes("Pixel_6_API_")).toArray();
  }
  return deviceList;
}
