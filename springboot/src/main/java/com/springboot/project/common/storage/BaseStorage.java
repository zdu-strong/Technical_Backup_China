...
public class BaseStorage {
    ...
    protected String getRelativePathFromRequest(HttpServletRequest request) {
        try {
            ...
            ResourceAccessLegalModel resourceAccessLegalModel = JSON.parseObject(
    ...
    public String getResoureUrlFromResourcePath(String relativePathOfResource) {
        try {
            ...
            String rootFolderName = JinqStream.from(Lists.newArrayList(StringUtils.split(relativePath, "/"))).limit(1)
                    .getOnlyValue();
            ...
            pathSegmentList
                    .add(Base64.getUrlEncoder().encodeToString(this.encryptDecryptService.getAES().encrypt(
                            JSON.toJSONString(resourceAccessLegalModel))));
