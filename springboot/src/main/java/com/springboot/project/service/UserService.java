...
public class UserService extends BaseService {
    ...
    public UserModel getNewAccountForSignUp() {
        ...
        user.setUserSignInVerificationCodeList(Lists.newArrayList());
    ...
    public UserModel getAccountForSignUp(String userIdOrEmail) {
        var user = this.UserEntity().where(s -> s.getId().equals(userIdOrEmail) || JinqStream.from(s.getUserEmailList())
                .where(m -> m.getDeleteKey().equals("")).where(m -> m.getEmail().equals(userIdOrEmail)).count() > 0)
    ...
    public void checkExistAccount(String userIdOrEmail) {
        ...
        stream = stream.where(s -> s.getId().equals(userId) || JinqStream.from(s.getUserEmailList())
                .where(m -> m.getDeleteKey().equals("")).where(m -> m.getEmail().equals(email)).count() > 0);
