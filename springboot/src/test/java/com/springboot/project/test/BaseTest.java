...
public class BaseTest {
    ...
    protected TokenModel createAccount(String email) {
        try {
            ...
            return signIn(email);
        } catch (URISyntaxException | InvalidKeySpecException | NoSuchAlgorithmException e) {
    ...
    private void signUp(UserModel userModelOfNewAccount, String email)
        ...
        userModelOfSignUp
                .setPrivateKeyOfRSA(this.encryptDecryptService
                        .encryptByAES(JSON.toJSONString(map)));
    ...
    private boolean existUser(String email) {
        try {
            this.userService.getAccountForSignUp(email);
            return true;
        } catch (Throwable e) {
            return false;
        }
    ...
    private TokenModel signIn(String email)
        ...
        }
        {
            ...
            userModel.setPrivateKeyOfRSA(newRSA.encryptBase64(JSON
                    .parseObject(this.encryptDecryptService
                            .decryptByAES(user.getPrivateKeyOfRSA()))
                    .getString("privateKeyOfRSA"), KeyType.PublicKey));
            ...
            this.testRestTemplate.getRestTemplate()
                    .setInterceptors(Collections.singletonList(new HttpHeaderInterceptor(HttpHeaders.AUTHORIZATION,
                            "Bearer " + accessToken)));
            var tokenModel = new TokenModel();
            tokenModel.setUserModel(getUserInfo(accessToken));
            tokenModel.setAccess_token(accessToken);
            tokenModel.setRSA(new RSA((RSAPrivateKey) KeyFactory.getInstance("RSA")
                    .generatePrivate(new PKCS8EncodedKeySpec(
                            Base64.getDecoder().decode(new RSA(keyPair.getPrivate(), null)
                                    .decryptStr(tokenModel.getUserModel().getPrivateKeyOfRSA(), KeyType.PrivateKey)))),
                    null));
            return tokenModel;
        }
    }
