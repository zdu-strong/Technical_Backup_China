    ...
    this.OrganizeEntity().select(
        (s, t) -> new Pair<>(s,
                t.stream(OrganizeEntity.class)
                        .where(m -> Boolean.valueOf(true)
                                .equals(MysqlFunction.isChildOrganize(m.getId(), s.getId())))
                        .count()))
        .group(s -> s.getOne(), (s, t) -> t.sumLong(m -> m.getTwo()))
        .sortedBy(s -> s.getTwo())
        .toList();
...
## Notes - Accept timezone from javascript, then convert to zoneOffset. zoneOffset can be passed to the mysql as a parameter
